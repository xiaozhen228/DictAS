        ...
        for data_name in self.dataset_name:
            ...
            for mode in ["train","test"]:
                ...
                if len(defect_classes) == 1 and "good" in defect_classes and mode=="train":
                    data_list = glob.glob(os.path.join(data_path,mode,"good", "rgb", '*.png'))
                    ...
                    for i in range(len(data_list)):
                        ...
                        save_img_path = os.path.join(des_path_root,new_class_name,"train","good","mvtec3D_"+str(id).zfill(6)+".bmp")  
                        cv2.imwrite(save_img_path,raw_img)
                        id = id + 1
                else:
                    if len(defect_classes) > 1 and mode == "test":
                        for classes in defect_classes:
                            data_list = glob.glob(os.path.join(data_path,mode,classes,"rgb", '*.png'))
                            data_list = sorted(data_list)
                            if classes == "good":
                                for i in range(len(data_list)):
                                    ...
                                    save_img_path = os.path.join(des_path_root,new_class_name,"test","good","mvtec3D_"+"good_"+ str(img_name_id)+ "_" +str(id).zfill(6)+".bmp") 
                                    cv2.imwrite(save_img_path,raw_img)
                                    id = id + 1
                            else:
                                for i in range(len(data_list)):
                                    img_name_id = os.path.basename(data_list[i]).split(".")[0]
                                    raw_mask_path = os.path.join(data_path,mode, classes, "gt", os.path.basename(data_list[i]))
                                    raw_img = cv2.imread(data_list[i],cv2.IMREAD_COLOR)
                                    raw_mask = cv2.imread(raw_mask_path,cv2.IMREAD_GRAYSCALE)
                                    if binary:
                                    ...
                                    new_class_name = data_name
                                    save_img_path = os.path.join(des_path_root,new_class_name,"test","anomaly",f"mvtec3D_{classes}_"+ str(img_name_id) + "_" + str(id).zfill(6)+".bmp")
                                    save_mask_path = os.path.join(des_path_root,new_class_name,"ground_truth","anomaly",f"mvtec3D_{classes}_"+ str(img_name_id) + "_" + str(id).zfill(6)+".png")

                                    cv2.imwrite(save_img_path,raw_img)
                                    #cv2.imwrite(save_mask_path,raw_mask, [int(cv2.IMWRITE_PNG_COMPRESSION),0])
                                    cv2.imwrite(save_mask_path,raw_mask)
                                    id = id + 1
        print("mvtec 3D finished !")
        return id 
